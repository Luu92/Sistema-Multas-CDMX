...
public class FXMLCitasController implements Initializable {
    ...
    @FXML
    private TableColumn<ModeloTabla,String> columna_lugar;
   ...
   */
   public void mandarRegistroBD(){
     ...
     alta= "INSERT INTO tregistro(hora,fecha,verificentro) VALUES (?,?,?)";
     try {
         PreparedStatement pst = cn.prepareStatement(alta);
         pst.setString(1,lblHora.getText());
         pst.setString(2,lblcitaFecha.getText());
         pst.setString(3,lblDelegacionYvereficentro.getText());
         pst.execute();
         JOptionPane.showMessageDialog(null,"Fecha, hora y verificentro agregada con exito");
     } catch (Exception e) {
	...
	public void mostrarTabla(){   
        try {
            ...
            while(rs.next()){
                oblist.add(new ModeloTabla(rs.getString("numCita"),rs.getString("fecha"),rs.getString("hora"),rs.getString("verificentro")));
            }
        } catch (SQLException ex) {
